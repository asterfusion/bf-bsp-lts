...
uint32_t *FAST_FUNC crc32_filltable (
    uint32_t *crc_table, int endian)
{
    uint32_t polynomial = endian ? 0x04c11db7 :
                          0xedb88320;
    uint32_t c;
    int i, j;
...
uint32_t FAST_FUNC crc32_block_endian1 (
    uint32_t val,
    const void *buf,
    unsigned len,
    uint32_t *crc_table)
...
uint32_t FAST_FUNC crc32_block_endian0 (
    uint32_t val,
    const void *buf,
    unsigned len,
    uint32_t *crc_table)
{
    ...
    while (buf != end) {
        val = crc_table[ (uint8_t)val ^ * (uint8_t *)buf]
              ^ (val >> 8);
        buf = (uint8_t *)buf + 1;
    }
    return val;
                     ...
                     const unsigned char *buf, unsigned len)
{
    ...
    return crc32_block_endian0 (crc ^ 0xffffffffL,
                                buf, len, global_crc32_table) ^
           0xffffffffL;
}

static inline int is_multicast_ether_addr (
    const u_int8_t *addr)
{
    return 0x01 & addr[0];
}

static inline int is_zero_ether_addr (
    const u_int8_t *addr)
{
...
static inline int is_valid_ether_addr (
    const u_int8_t *addr)
{
...
static int set_bytes (char *buf,
                      const char *string, int *converted_accum)
{
    char *p = (char *)string;
    int i;
    uint byte;
    ...
    for (i = 0, p = (char *)string;
         (i < TLV_VALUE_MAX_LEN) && (*p != 0); i++) {
        while ((*p == ' ') || (*p == '\t') ||
               (*p == ',') || (*p == ';')) {
            p++;
        }
 ...
 */
static inline bool is_valid_tlv (tlvinfo_tlv_t
                                 *tlv)
{
 ...
 *
 */
static inline bool is_valid_tlvinfo_header (
    tlvinfo_header_t *hdr)
{
    int max_size = TLV_TOTAL_LEN_MAX;
    return ((strcmp (hdr->signature,
                     TLV_INFO_ID_STRING) == 0) &&
...
static void decode_tlv_value (tlvinfo_tlv_t *tlv,
                              char *value)
{
    int i;
    ...
    switch (tlv->type) {
        ...
        case TLV_CODE_VENDOR_EXT:
            value[0] = 0;
            for (i = 0;
                 (i < (TLV_DECODE_VALUE_MAX_LEN / 5)) &&
                 (i < tlv->length);
                 i++) {
                sprintf (value, "%s 0x%02X", value,
                         tlv->value[i]);
            }
            break;
...
static void decode_tlv (tlvinfo_tlv_t *tlv)
{
    ...
    strncpy (name, tlv_type2name (tlv->type),
             DECODE_NAME_MAX);
...
{
    tlvinfo_header_t *eeprom_hdr = (tlvinfo_header_t
                                    *)eeprom;
    ...
    eeprom_crc = (tlvinfo_tlv_t *)&eeprom[sizeof (
                   tlvinfo_header_t) +
    ...
    calc_crc =
        crc32 (0,
               (void *)eeprom,
               sizeof (tlvinfo_header_t) + be16_to_cpu (
                   eeprom_hdr->totallen) - 4);
...
static void update_crc (u_int8_t *eeprom)
{
    ...
    if (!tlvinfo_find_tlv (eeprom, TLV_CODE_CRC_32,
                           &eeprom_index)) {
        ...
        eeprom_index = sizeof (tlvinfo_header_t) +
                       be16_to_cpu (eeprom_hdr->totallen);
        eeprom_hdr->totallen = cpu_to_be16 (be16_to_cpu (
                                                eeprom_hdr->totallen) +
                                            sizeof (tlvinfo_tlv_t) + 4);
    }
    eeprom_crc = (tlvinfo_tlv_t *)
                 &eeprom[eeprom_index];
...
void show_eeprom (u_int8_t *eeprom)
{
    ...
    tlv_end = sizeof (tlvinfo_header_t) +
              be16_to_cpu (eeprom_hdr->totallen);
                       ...
                       u_int8_t tcode, int *eeprom_index)
{
    ...
    *eeprom_index = sizeof (tlvinfo_header_t);
    ...
    while (*eeprom_index < eeprom_end) {
        ...
        *eeprom_index += sizeof (tlvinfo_tlv_t) +
                         eeprom_tlv->length;
    }
    return (FALSE);
}
                         ...
                         u_int8_t tcode, char *value)
{
    ...
    if (tlvinfo_find_tlv (eeprom, tcode,
                          &eeprom_index)) {
        eeprom_tlv = (tlvinfo_tlv_t *)
                     &eeprom[eeprom_index];
